...
- 📡 **多协议代理** — 支持 TCP / UDP / XTCP（P2P 打洞）/ HTTP / HTTPS 代理
- 🌐 **HTTP/HTTPS 域名映射** — 基于域名的 vhost 代理，多站点共享 80/443 端口
- 🔒 **HTTP Basic Auth** — vhost 代理支持访问认证
- 🏷️ **子域名支持** — 自动拼接子域名，无需手动配置完整域名
- 📋 **实时状态监控** — 服务运行状态、连接数、日志实时查看
- 📦 **单文件分发** — 打包为独立 EXE，无需安装 Python 环境
- 🚀 **轻量高效** — 纯 Python 实现 frp 协议，无额外依赖
- 🖼️ **系统托盘** — 客户端支持最小化到系统托盘
...
2. 在「服务配置」面板中设置：
   ...
   - **绑定端口**：默认 `7000`
   - **Token**：点击「随机生成」或手动输入
   - **HTTP 端口**：HTTP vhost 监听端口（默认 0 不启用，设为 80 启用）
   - **HTTPS 端口**：HTTPS vhost 监听端口（默认 0 不启用，设为 443 启用）
   - **证书文件**：HTTPS 证书 PEM 文件路径
   - **私钥文件**：HTTPS 私钥 PEM 文件路径
   - **子域名根**：如 `frp.example.com`
...
2. 填写服务端连接信息：
   ...
   - **服务器端口**：默认 `7000`
   - **Token**：与服务端配置一致
3. 添加代理规则（支持 TCP / UDP / XTCP / HTTP / HTTPS）
4. 点击「连接」开始内网穿透
